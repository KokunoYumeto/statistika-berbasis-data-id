...
# Airport records: OpenIntroStat/openintro commit
# 48793d9645e0da033daaca1c1a19a051533d79d2 (package version 2.0.1),
# the last official package commit before this book figure was added.
# Deterministic non-R replay: regenerate_id.py
# The bundled Census metadata requires visible U.S. Census Bureau
# acknowledgement in any product or publication using this base map.
...
usairports <- usairports %>%
  ...
  mutate(
    ownership = case_when(
      ownership == "PR" ~ "Milik swasta",
      ownership == "PU" ~ "Milik publik"
    ),
    use = case_when(
      use == "PR" ~ "Penggunaan privat",
      use == "PU" ~ "Penggunaan publik"
    ),
    ownership = factor(ownership, levels = c("Milik swasta", "Milik publik")),
    use = factor(use, levels = c("Penggunaan privat", "Penggunaan publik")),
    region = case_when(
      region == "AAL" ~ "Alaska",
      region == "ACE" ~ "Central",
      region == "AEA" ~ "Eastern",

#!/usr/bin/env python3
"""Resolve exact R011-B024 release inputs without publishing anything."""
...
def build_config() -> dict:
    ...
    config = {
        ...
        "next_cursor": {
            ...
            "authority_line": 2008,
            "source_label": "twoWayTablesAndChiSquare",
            "source_order": "Bab 6, Bagian 6.4",
        },
        "source_package": {
...
def main() -> int:
    ...
    if args.self_check:
        ...
        print(json.dumps(static_self_check("b024-release-preparer"), ensure_ascii=False, sort_keys=True))
        return 0
    if args.dry_run:
        if args.replace:
            parser.error("--replace is valid only with --prepare")
        result = static_self_check("b024-release-preparer-dry-run")
        result.update(
            {
    ...
    if CONFIG_PATH.exists() and not args.replace:
        raise ReleaseGateError("B024 RELEASE_INPUTS.json already exists; inspect it before --replace")
    config = build_config()
    ...
    print(
        json.dumps(
            {
                "$schema": "r011-b024-release-preparation-receipt/v1",
                "boundary_id": BOUNDARY_ID,
                "release_id": RELEASE_ID,

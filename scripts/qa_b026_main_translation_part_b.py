#!/usr/bin/env python3
"""Deterministically audit staged R011-B026 main translation Part B."""
...
TARGET_REL = Path("qa/b026-translation/staging/chapter-lines-232-400.id.tex")
BLUEPRINT_REL = Path("qa/b026-source/R011-B026_BOUNDARY_BLUEPRINT.json")
RECEIPT_REL = Path("qa/b026-translation/R011-B026_MAIN_TRANSLATION_PART_B_QA.json")
...
INDONESIAN_ANCHORS = [
    "Dalam praktiknya",
    "pemeriksaan secara mental",
    "kemencengan sedang",
    "pencilan yang sangat ekstrem",
    "puluhan juta pengikut",
    "distribusinya sangat menceng",
    "syarat normalitas",
    "Teorema Limit Pusat",
]
...
RESIDUAL_ENGLISH_PATTERNS = [
    r"\bIn practice\b",
    r"\bmental check\b",
    r"\bunderlying population\b",
    r"\bmoderate skew\b",
    r"\bparticularly extreme outliers\b",
    r"\bbeyond what we observe\b",
    r"\bnumber of followers\b",
    r"\bindividual account\b",
    r"\blarge majority of accounts\b",
    r"\bcouple thousand followers\b",
    r"\brelatively tiny fraction\b",
    r"\btens of millions of followers\b",
    r"\bextremely skewed\b",
    r"\bsample size is large enough\b",
...
def overlapping_blueprint_repairs(blueprint: dict[str, object]) -> list[dict[str, object]]:
...
def build_receipt() -> dict[str, object]:
    ...
    chunk = blueprint["translation_chunks"][2]
    if (chunk["first_line"], chunk["last_line"]) != (FIRST_LINE, LAST_LINE):
        raise AssertionError("blueprint Part B chunk boundary drift")
    repair_candidates = overlapping_blueprint_repairs(blueprint)
    if repair_candidates:
        raise AssertionError("unexpected approved repair overlaps Part B")
    ...
    if len(source_lines_bytes) != expected_line_count or len(target_lines_bytes) != expected_line_count:
    ...
    if sha256(source_slice) != chunk["sha256"] or len(source_slice) != chunk["bytes"]:
        raise AssertionError("blueprint Part B source identity drift")
    ...
    receipt: dict[str, object] = {
        ...
        "target": {
            **target_identity,
            ...
            "mapping": "target line 1 maps to source line 232; target line 169 maps to source line 400",
        },
        "qa": {
        ...
        "next_translation_cursor": {
            "path": SOURCE_REL.as_posix(),
            "line": NEXT_LINE,
            "blueprint_chunk": "401-633 inclusive",
            "context": (
                "Introduction to the t-distribution, degrees of freedom, tools, and tail examples."
            ),
        },
        "scope_guards": {
...
def main() -> None:
    ...
    if args.write:
        ...
        if not RECEIPT.exists() or RECEIPT.read_bytes() != first:
            raise AssertionError("on-disk B026 Part B QA receipt differs from exact replay")

    result = {
        "status": "PASS_EXACT_REPLAY_R011_B026_MAIN_TRANSLATION_PART_B",

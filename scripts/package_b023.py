#!/usr/bin/env python3
"""Build a deterministic, reader-first R011-B023 release package offline."""
...
SOURCE_TOOLING = (
    "scripts/build_b023_boundary_clean_reader.py",
    "scripts/localize_b023_blade_chart.py",
    "scripts/audit_b023_translation.py",
    "scripts/qa_b023_boundary_clean_reader.py",
)
...
def _citation(config: dict) -> bytes:
    return (
        ...
        "title: \"Statistika Berbasis Data - Edisi Kerja Bahasa Indonesia (R011-B023)\"\n"
        "type: book\n"
...
def _licenses(config: dict) -> bytes:
    return (
        ...
        "atribusi CC BY 2.0 yang diwarisi. Gambar bagan bilah yang dilokalkan "
        "mempertahankan asal dan skrip penghasil hulunya.\n\n"
...
def _readme(config: dict) -> bytes:
    return (
        "# Statistika Berbasis Data - R011-B023\n\n"
        "Edisi kerja Bahasa Indonesia ini masih **parsial**, tetapi pembaca saat "
        "ini merupakan satu batas yang koheren dan bersih: 241 halaman yang "
        "diterima sampai Bab 6, Bagian 6.2 (Selisih dua proporsi). Semua 241 "
        ...
        "Cakupan latihan Bagian 6.1-6.2 adalah nomor 1-30. Jawaban publik hulu "
        "tersedia untuk nomor ganjil 1-29; nomor genap 2-30 dicatat sebagai "
...
def zenodo_metadata(config: dict) -> dict:
    description = (
        ...
        "dengan pembaca bersih 241 halaman hingga Bab 6, Bagian 6.2 (Selisih "
        "dua proporsi). Tiga puluh latihan Bagian 6.1-6.2 disertakan; jawaban "
        "publik tersedia untuk nomor ganjil 1 sampai 29 dan kesenjangan O001 "
        "dicatat untuk nomor genap 2 sampai 30. Tidak ada solusi instruktur "
    ...
    return {
        "metadata": {
            "title": "Statistika Berbasis Data - Edisi Kerja Bahasa Indonesia (R011-B023: hingga Bagian 6.2)",
            ...
            "notes": (
                "Partial working edition; exact checkpoint R011-B023. Reader SHA-256 "
...
def _public_backend_manifest(config: dict, public_rows: list[dict], exclusions: list[dict]) -> bytes:
    return canonical_json_bytes(
        {
            "$schema": "r011-b023-public-backend-projection-manifest/v1",
...
def package(*, replace: bool) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="r011-b023-package-") as temp_name:
        temp = Path(temp_name)
        ...
        if identity(temp / reader_name)["sha256"] != config["inputs"]["reader"]["sha256"]:
            raise ReleaseGateError("copied B023 reader differs from bound reader")
        ...
        manifest = {
            "$schema": "r011-b023-release-manifest/v1",
            "boundary_id": BOUNDARY_ID,
            "release_id": RELEASE_ID,
            "version": config["version"],
        ...
        if existing and not replace:
            raise ReleaseGateError("B023 package targets exist; inspect them before --replace: " + ", ".join(existing))
        RELEASE_DIR.mkdir(parents=True, exist_ok=True)
        for name in ORDERED_RELEASE_ASSETS:
            (temp / name).replace(RELEASE_DIR / name)
    verified = verify_release_package(config)
    return {
        "$schema": "r011-b023-package-receipt/v1",
        "boundary_id": BOUNDARY_ID,
        "release_id": RELEASE_ID,
...
def main() -> int:
    ...
    if args.self_check:
        print(json.dumps(static_self_check("b023-packager"), ensure_ascii=False, sort_keys=True))
        return 0
    if args.dry_run:
        result = static_self_check("b023-packager-dry-run")

#!/usr/bin/env python3
"""Publish verified R011-B024 bytes to the existing Zenodo/GitHub lineages.
...
from package_b024 import SOURCE_TOOLING, zenodo_metadata
...
ZENODO_RECEIPT_PATH = (
    REPO_ROOT / "qa" / "b024-publication" / f"ZENODO_PUBLICATION_RECEIPT_{RELEASE_ID}.json"
)
...
def _validate_zenodo_metadata(config: dict, value: dict) -> dict:
    ...
    checks = {
        ...
        "reader_pages": "253 halaman" in description,
        "scope": "Bagian 6.3" in description,
        "exercise_closure": "nomor ganjil 1 sampai 33" in description and "nomor genap 2 sampai 34" in description,
...
def _verify_zenodo_prior(config: dict, versions: list[dict]) -> None:
    ...
    if (
        ...
        raise ReleaseGateError("Zenodo public head no longer matches pinned B023")
...
def _zenodo_receipt(config: dict, record_id: int, readback: dict, status: str) -> dict:
    receipt = {
        "$schema": "r011-b024-zenodo-publication-receipt/v1",
...
def publish_zenodo(config: dict) -> dict:
    ...
    if len(matches) > 1:
        raise ReleaseGateError("multiple Zenodo records claim the B024 version")
    if matches:
    ...
    if int(prior.get("id", prior.get("record_id", -1))) != prior_id or int(prior.get("conceptrecid", -1)) != 22059801 or prior.get("state") != "done":
        raise ReleaseGateError("Zenodo authenticated B023 predecessor changed")
    ...
    if draft_id < 0 or int(draft.get("conceptrecid", -1)) != 22059801:
        raise ReleaseGateError("Zenodo B024 draft identity is malformed")
    bucket = draft.get("links", {}).get("bucket")
    if not isinstance(bucket, str) or not bucket.startswith("https://zenodo.org/api/files/"):
        raise ReleaseGateError("Zenodo B024 draft lacks fixed upload bucket")
    ...
    if any(row.get("metadata", {}).get("version") == VERSION for row in guard):
        raise ReleaseGateError("B024 Zenodo version appeared concurrently before publish")
...
def _github_values(config: dict, zenodo: dict) -> dict[str, bytes]:
    ...
    for path in SOURCE_TOOLING + (
        "scripts/release_b024_common.py",
        "scripts/prepare_b024_release.py",
        "scripts/package_b024.py",
        "scripts/publish_b024.py",
    ):
    ...
    projection = {
        "$schema": "r011-b024-github-public-backend-projection/v1",
        "boundary_id": BOUNDARY_ID,
        "release_id": RELEASE_ID,
    ...
    add(
        "00_control/PUBLICATION_STATE_R011-B024.md",
        (
            "# Status publikasi R011-B024\n\n"
            "Edisi kerja parsial: pembaca bersih 253 halaman hingga Bab 6 Bagian 6.3; "
            "nol halaman prosa instruksional/latihan/jawaban publik berbahasa Inggris. "
            "Latihan 1-34; jawaban publik ganjil 1-33; kesenjangan O001 genap 2-34. "
    ...
    add(
        "00_control/PUBLICATION_CURSOR_R011-B024.json",
        canonical_json_bytes(
            {
                "$schema": "r011-b024-github-publication-cursor/v1",
                "boundary_id": BOUNDARY_ID,
                "release_id": RELEASE_ID,
...
def _github_release_contract(config: dict, zenodo: dict) -> dict:
    return {
        ...
        "name": "R011-B024 - pembaca Bahasa Indonesia hingga Bagian 6.3",
        "body": (
            "Rilis kerja parsial dengan pembaca bersih 253 halaman hingga Bab 6, "
            "Bagian 6.3. Latihan 1-34 disertakan; jawaban publik ganjil 1-33; "
            "kesenjangan O001 genap 2-34. Korpus lengkap: tidak. Nol halaman prosa "
...
def _github_public_download(url: str, wanted: dict) -> dict:
    ...
    session.headers["User-Agent"] = "interlanguage-r011-b024-public-readback/1"
...
def _create_github_tree_commit(client: GitHubClient, config: dict, values: dict[str, bytes]) -> tuple[str, dict[str, str]]:
    ...
    if head != gh["prior_release_commit"]:
        ...
        if len(parents) != 1 or parents[0].get("sha") != gh["prior_release_commit"] or not _is_sha1(tree_sha):
            raise ReleaseGateError("GitHub main no longer has pinned B023 parent")
        ...
        if set(observed) != set(desired) or any(observed[path].get("sha") != digest for path, digest in desired.items()):
            raise ReleaseGateError("GitHub interrupted commit tree differs from desired B024 tree")
        return head, desired
    ...
    if current != head:
        raise ReleaseGateError("GitHub main advanced during B024 tree construction")
    commit = client.json(
        "POST",
        "git/commits",
        expected=(201,),
        payload={
            "message": "Preserve Indonesian R011-B024 reader through Section 6.3",
            "tree": tree_sha,
            "parents": [head],
...
def publish_github(config: dict) -> dict:
    if not ZENODO_RECEIPT_PATH.is_file():
        raise ReleaseGateError("verified B024 Zenodo publication receipt is absent")
    ...
    if tag is not None:
        ...
        if not _is_sha1(commit_sha):
            raise ReleaseGateError("GitHub existing B024 tag is malformed")
    ...
    if len(parents) != 1 or parents[0].get("sha") != gh["prior_release_commit"]:
        raise ReleaseGateError("GitHub B024 release commit parent changed")
    receipt = {
        "$schema": "r011-b024-github-publication-receipt/v1",
        "boundary_id": BOUNDARY_ID,
        "release_id": RELEASE_ID,
        "status": status,
...
def main(destination: str) -> int:
    parser = argparse.ArgumentParser(description=f"Bounded R011-B024 {destination} publisher")
    ...
    if args.self_check:
        print(json.dumps(static_self_check(f"b024-{destination}-publisher"), ensure_ascii=False, sort_keys=True))
        return 0
    if args.dry_run:
        result = static_self_check(f"b024-{destination}-publisher-dry-run")
        result.update(
            {

...
SOURCE_SHA256 = "a3f468ed9b4d7529f4c7da909133cb0bd21e341db849a6efb200304aa7af61d7"
RELATIVE_SOURCE = Path("ch_intro_to_data/figures/multiunitsVsOwnership/multiunitsVsOwnership.pdf")
OUTPUT = Path(__file__).resolve().with_name("multiunitsVsOwnership.pdf")
REMOVE = {"Homeownership Rate", "Percent of Units in Multi-Unit Structures"}
...
def make_overlay(width: float, height: float) -> io.BytesIO:
    ...
    c.rotate(90)
    c.drawCentredString(0, 0, "Tingkat Kepemilikan Rumah")
    c.restoreState()
    c.setFont("Helvetica", 8.5)
    c.drawCentredString(280.0, 4.32, "Persentase Unit Hunian dalam Bangunan Multiunit")
    c.showPage()
    c.save()
    buffer.seek(0)
    return buffer
...
def main() -> None:
    ...
    with pikepdf.Pdf.open(source) as pdf:
        ...
        temporary = OUTPUT.with_name("multiunitsVsOwnership.tmp-id.pdf")

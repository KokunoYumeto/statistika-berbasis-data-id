...
\begin{examplewrap}
\begin{nexample}{Kota New York memiliki sekitar 8 juta penduduk.
    Berapa orang yang diperkirakan akan dirawat di rumah sakit
    akibat infark miokard akut (acute myocardial infarction, AMI), yaitu serangan jantung,
    setiap hari?
    Menurut catatan historis, jumlah rata-ratanya
    sekitar 4.4 orang.
    Namun, kita juga ingin mengetahui perkiraan
    distribusi banyaknya kasus.
    Seperti apakah histogram banyaknya kasus AMI
    per hari jika jumlah hariannya dicatat selama
    satu tahun penuh?}
  \label{amiIncidencesEachDayOver1YearInNYCExample}%
  Histogram banyaknya kasus AMI selama 365 hari
  di Kota New York ditampilkan pada
  Gambar~\ref{amiIncidencesOver100Days}.\footnotemark{}
  Rata-rata sampel (4.38) serupa dengan rata-rata historis
  sebesar~4.4.
  Simpangan baku sampel sekitar 2, dan histogram
  menunjukkan bahwa sekitar 70\% data berada di antara 2.4 dan~6.4.
  Bentuk distribusinya unimodal dan menceng kanan.
\end{nexample}
\end{examplewrap}
\footnotetext{Data ini disimulasikan. Dalam praktik, kita perlu memeriksa adanya hubungan di antara hari-hari yang berurutan.}

\begin{figure}[h]
  \centering
  \Figure[Sebuah histogram ditampilkan untuk ``Peristiwa AMI (per Hari)''. Ada 11 nilai bukan nol yang ditampilkan: frekuensi sekitar 15 pada nilai 1, frekuensi 50 pada nilai 2, 70 pada 3, 85 pada 4, 55 pada 5, 45 pada 6, 25 pada 7, 20 pada 8, 5 pada 9, 5 pada 10, dan frekuensi sekitar 2 pada 11.]{0.6}{amiIncidencesOver100Days}
  \caption{Histogram banyaknya kasus AMI
      pada 365 hari yang berbeda di Kota New York.}
  \label{amiIncidencesOver100Days}
\end{figure}

\termsub{Distribusi Poisson}{distribution!Poisson} sering berguna untuk memperkirakan banyaknya peristiwa dalam suatu populasi besar selama satu satuan waktu. Sebagai contoh, perhatikan setiap peristiwa berikut:
\begin{itemize}
\setlength{\itemsep}{0mm}
\item mengalami serangan jantung,
\item menikah, dan
\item tersambar petir.
\end{itemize}
...
Histogram pada Gambar~\ref{amiIncidencesOver100Days} menghampiri distribusi Poisson dengan laju 4.4. \term{Laju} suatu distribusi Poisson adalah rata-rata banyaknya peristiwa dalam populasi yang relatif tetap per satuan waktu. Pada Contoh~\ref{amiIncidencesEachDayOver1YearInNYCExample}, satuan waktunya adalah satu hari, populasinya adalah seluruh penduduk Kota New York, dan laju historisnya adalah 4.4. Parameter distribusi Poisson adalah lajunya -- yaitu banyaknya peristiwa yang kita harapkan akan teramati -- dan biasanya dilambangkan dengan $\lambda$\index{Greek!lambda@lambda ($\lambda$)}
(huruf Yunani \emph{lambda}) atau $\mu$. Dengan menggunakan laju tersebut, kita dapat menggambarkan peluang mengamati tepat $k$ peristiwa dalam satu satuan waktu.

\D{\newpage}

\begin{onebox}{Distribusi Poisson}
  Misalkan kita mengamati kemunculan peristiwa dan banyaknya
  peristiwa yang teramati mengikuti distribusi Poisson
  dengan laju $\lambda$.
  Maka
  \begin{align*}
  P(\text{mengamati $k$ peristiwa})
      = \frac{\lambda^{k} e^{-\lambda}}{k!}
  \end{align*}
  dengan $k$ dapat bernilai 0, 1, 2, dan seterusnya,
  serta $k!$ menyatakan $k$-faktorial, sebagaimana dijelaskan pada
  halaman~\pageref{factorial_defined}.
  Huruf $e\approx2.718$ adalah basis logaritma
  natural.
  Rata-rata dan simpangan baku distribusi ini
  masing-masing adalah $\lambda$ dan $\sqrt{\lambda}$.
\end{onebox}
...
Bahkan ketika peristiwa-peristiwa tersebut sebenarnya tidak independen --
misalnya, hari Sabtu dan Minggu sangat
populer untuk pernikahan --
model Poisson terkadang masih cukup tepat
jika kita membolehkan lajunya berbeda untuk waktu yang
berbeda.
Dalam contoh pernikahan, lajunya akan dimodelkan
lebih tinggi pada akhir pekan daripada pada hari kerja.
Gagasan memodelkan laju distribusi Poisson
sebagai fungsi dari variabel kedua, seperti hari dalam sepekan, menjadi
dasar bagi sejumlah metode lanjutan
yang termasuk dalam ranah \termsub{model linear terampat}
    {generalized linear model}.
Pada Bab~\ref{linRegrForTwoVar}
dan~\ref{multipleAndLogisticRegression},
kita akan membahas dasar-dasar model linear.

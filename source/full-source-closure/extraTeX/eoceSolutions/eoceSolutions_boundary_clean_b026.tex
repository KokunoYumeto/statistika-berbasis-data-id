...
\eocesol{(a)~Salah. Distribusi khi-kuadrat memiliki satu parameter yang disebut derajat 
kebebasan.
(b)~Benar.
(c)~Salah. Statistik khi-kuadrat selalu nonnegatif, bukan selalu positif; nilainya dapat sama dengan nol ketika setiap frekuensi teramati sama dengan frekuensi harapannya.
(d)~Salah. Ketika derajat kebebasan meningkat, bentuk distribusi khi-kuadrat 
menjadi semakin simetris.}

% 33

\eocesol{(a)~$H_0$: Distribusi format buku yang digunakan mahasiswa 
mengikuti prediksi dosen. $H_A$: Distribusi format buku yang 
digunakan mahasiswa tidak mengikuti prediksi dosen.
(b)~$E_{hard~copy} = 126 \times  0.60 = 75.6$. 
$E_{print} = 126 \times  0.25 = 31.5$.
$E_{online} = 126 \times  0.15 = 18.9$.
(c)~Independensi: Sampel tersebut tidak acak. Namun, jika dosen memiliki 
alasan untuk meyakini bahwa proporsi tersebut stabil dari satu semester ke semester berikutnya 
dan mahasiswa tidak saling memengaruhi kebiasaan belajar, independensi 
mungkin merupakan asumsi yang wajar. Ukuran sampel: Semua frekuensi harapan sekurang-kurangnya 5.
(d)~$\chi^2 = 2.32$, $df=2$, nilai-p = 0.313.
(e)~Karena nilai-p besar, kita gagal menolak $H_0$. Data tidak 
memberikan bukti kuat yang menunjukkan bahwa prediksi dosen tersebut 
tidak akurat secara statistik.}

#!/usr/bin/env python3
"""Generate the terminal deterministic R011-B013 backend append.

The admitted R011-B012 backend is an immutable, hash-bound preimage.  This
program writes only the explicit isolated ``qa/b013-backend-final`` stages; it
...
STAGE_ROOT = LANE / "qa" / "b013-backend-final"
sys.path.insert(0, str(SCRIPTS))
import generate_backend_b012 as b012  # noqa: E402

g = b012.g
RECORD_PATHS = b012.RECORD_PATHS

BOUNDARY_ID = "R011-B013"
BASE_BOUNDARY_ID = "R011-B012"
SCHEMA_VERSION = "0.1.0"
RECORDED_AT = "2026-08-24T22:00:00+02:00"
WORKFLOW_ID = "r011-openintro-statistics-id-b013-backend-final"
...
TERMINAL_CONTRACT = (
    LANE / "qa" / "b013-backend-prep" / "R011-B013_TERMINAL_INPUTS.json"
)
REQUIREMENTS = (
    LANE / "qa" / "b013-backend-prep" / "R011-B013_BACKEND_INPUT_REQUIREMENTS.json"
)
...
EOCE_AUTHORITY = AUTHORITY_ROOT / "ch_probability" / "TeX" / "continuous_distributions.tex"
ANSWER_AUTHORITY = AUTHORITY_ROOT / "extraTeX" / "eoceSolutions" / "eoceSolutions.tex"

RUN_A = STAGE_ROOT / "run-a"
RUN_B = STAGE_ROOT / "run-b"
VALIDATION_RECEIPT = STAGE_ROOT / "R011-B013_BACKEND_VALIDATION_RECEIPT.json"

BASE_MANIFEST_IDENTITY = {
    "bytes": 57355,
    "sha256": "679f48f4d1170a2dd4cfdf164b346827ed76f6e516285c4691de3454dce79fcb",
}
BASE_INVENTORY_IDENTITY = {
    "files": 282,
    "bytes": 103636392,
    "sha256": "f9049b89169d4f6314a0bb2ee3d254c74b1a5622f101bdc14df421022fde9156",
}
BASE_RECORD_COUNT = 4276
TERMINAL_CONTRACT_IDENTITY = {
    "bytes": 6253,
    "sha256": "3ab502dfe6767faabb5168b026e711db1129f1a667b1a8caffdff5e41113b1a7",
}
REQUIREMENTS_IDENTITY = {
    "bytes": 7608,
    "sha256": "9883d8d2086a5d948c76c525d04c1066450378de514808c6e823509a9f5ee1c4",
}

PUBLIC_ANSWERS = [37]
O001_GAPS = [38]
EOCE_EXERCISES = [37, 38]
GUIDED_EXERCISES = [1, 2]
WORKED_EXAMPLES = [1, 2, 3]

BASE_ADMISSION_RECEIPT = LANE / "qa" / "R011-B012_BOUNDARY_RECEIPT.json"
BASE_ADMISSION_RECEIPT_IDENTITY = {
    "bytes": 15232,
    "sha256": "8191d1fddc607578f0d35711c0680afc1c31b3f393254e2969e211cf679ee553",
}
BASE_ADMISSION_CHECKPOINT = LANE / "00_control" / "R011-B012_ADMISSION_CHECKPOINT.md"
BASE_ADMISSION_CHECKPOINT_IDENTITY = {
    "bytes": 1958,
    "sha256": "c863c0cfd7776db3ae4cec19811cb856bef8774b94347c9096cf6cef90b2cd91",
}
...
def load_base() -> tuple[dict[str, list[dict[str, Any]]], dict[str, bytes], dict[str, Any]]:
    ...
    if manifest.get("backend_name") != "r011-openintro-statistics-id-b012-final-isolated":
        raise RuntimeError("live base is not exact admitted B012")
    if sum(int(value) for value in manifest["record_counts"].values()) != BASE_RECORD_COUNT:
        raise RuntimeError("B012 base record count changed")
    ...
    if {key: observed_inventory[key] for key in ("files", "bytes", "sha256")} != BASE_INVENTORY_IDENTITY:
        raise RuntimeError("B012 base export inventory changed")
    ...
    for name, relative in RECORD_PATHS.items():
        ...
        if len(rows) != int(entry["records"]) or g.jsonl_bytes(rows) != raw:
            raise RuntimeError(f"noncanonical B012 typed payload: {relative}")
        records[name] = rows
    auxiliary = {
...
def load_terminal() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if contract.get("boundary_id") != BOUNDARY_ID or contract.get("status") != "READY_TERMINAL_INPUTS":
        raise RuntimeError("B013 terminal contract is not ready")
    closure = contract.get("closure", {})
    if (
        closure.get("worked_examples") != 3
        or closure.get("guided_exercises") != 2
        or closure.get("guided_inline_public_answers") != 2
        or closure.get("subsections") != 2
        ...
        raise RuntimeError("B013 terminal closure changed")
    if set(contract.get("inputs", {})) != {
        item["key"] for item in requirements["required_terminal_inputs"]
    }:
        raise RuntimeError("B013 terminal role closure changed")
...
def structural_spans(raw: bytes) -> list[dict[str, Any]]:
    """Losslessly partition Section 3.5 around examples, guides and answers."""
    blocks: list[tuple[int, int, str, int]] = []
    for begin, end_token, kind, expected in (
        (b"\\begin{examplewrap}", b"\\end{examplewrap}", "worked_example", len(WORKED_EXAMPLES)),
        (b"\\begin{exercisewrap}", b"\\end{exercisewrap}", "guided_exercise", len(GUIDED_EXERCISES)),
    ):
        cursor = count = 0
        while True:
            ...
            cursor = end
        if count != expected:
            raise RuntimeError(f"B013 {kind} topology changed: {count} != {expected}")
    cursor = count = 0
    ...
    while True:
        ...
        cursor = end
    if count != len(GUIDED_EXERCISES):
        raise RuntimeError(f"B013 inline-answer topology changed: {count} != {len(GUIDED_EXERCISES)}")
    blocks.sort()
    for left, right in zip(blocks, blocks[1:]):
        if left[1] > right[0]:
            raise RuntimeError("overlapping B013 structural blocks")
...
def subsection_spans(raw: bytes) -> list[dict[str, Any]]:
    ...
    if len(matches) != 2:
        raise RuntimeError(f"B013 subsection topology changed: {len(matches)} != 2")
    result = []
...
def load_context() -> dict[str, Any]:
    ...
    eoce_authority = require(EOCE_AUTHORITY, {"bytes": 2358, "sha256": "42de1d525152bc5e5f85112963fdf7e9e5117b4148434046cde2b6fa17100cfe"})
    ...
    main_source = main_authority[124925:132799]
    answer_source = answer_authority[24671:24794]
    if {"bytes": len(main_source), "sha256": sha256_bytes(main_source)} != {"bytes": 7874, "sha256": "3459e761d91681907a99c555bd7fe627c9a80ddcd3658b46fd04f6382c5cad0c"}:
        raise RuntimeError("B013 authority Section 3.5 slice changed")
    if {"bytes": len(answer_source), "sha256": sha256_bytes(answer_source)} != {"bytes": 123, "sha256": "59a1c0ce9597f94ceac7cbdc9a0cae6a06aa995850261f22ae259d2a2fbbc840"}:
        raise RuntimeError("B013 public answer authority slice changed")
    ...
    answer_body_start = answer_target.find(b"% 37")
    if answer_body_start < 0:
        raise RuntimeError("terminal B013 answer body start disappeared")
    ...
    if main_target_offset < 0 or assembled_main.find(main_target, main_target_offset + 1) >= 0:
        raise RuntimeError("terminal B013 main fragment assembly binding changed")
    if answer_target_offset < 0 or assembled_answers.find(answer_body, answer_target_offset + 1) >= 0:
        raise RuntimeError("terminal B013 answer fragment assembly binding changed")
    ...
    if [(row["kind"], row["number"]) for row in source_spans] != [(row["kind"], row["number"]) for row in target_spans]:
        raise RuntimeError("B013 structural source/target segmentation changed")
...
EXERCISE_LABELS = {
    37: "cat_weights", 38: "income_gender",
}

TERM_SPECS = [
    ("continuous distribution", "distribusi kontinu", []),
    ("continuous numerical variable", "variabel numerik kontinu", []),
    ("density", "kepadatan", []),
    ("hollow histogram", "histogram berongga", []),
    ("probability", "peluang", ["probabilitas"]),
    ("probability density function", "fungsi kepadatan peluang", ["fungsi densitas peluang"]),
]
...
def build_records() -> tuple[dict[str, list[dict[str, Any]]], dict[str, bytes], dict[str, Any]]:
    ...
    previous_section = one(records, "units", "r011/unit/source-label/randomVariablesSection")["id"]
    chapter_id = one(records, "units", "r011/unit/source-label/ch_probability")["id"]

    text_rights_key = "r011/rights/b013-localized-section-text"
    asset_rights_key = "r011/rights/b013-localized-figure-derivatives"
    text_rights = g.stable_id(text_rights_key)
    asset_rights = g.stable_id(asset_rights_key)
    for key, scope in (
        (text_rights_key, "B013 Indonesian Section 3.5 text, EoCE 37--38, public answer 37, and bounded provenance-bearing preface overlay."),
        (asset_rights_key, "B013 Indonesian label-only PDF derivatives for fdicHistograms, usHeightsHist180185, fdicHeightContDist, fdicHeightContDistFilled, and cat_weights, with deterministic per-figure producer bindings."),
    ):
        records["rights"].append(record(
            "rights", key, component_scope=scope, license_expression="CC-BY-SA-3.0",
            verification_status="verified against R011-B013_ASSET_CLOSURE.json",
            attribution="OpenIntro Statistics source authors; Indonesian derivative changes identified by R011-B013.",
            ...
            publication_effect="Isolated terminal B013 backend; admission and publication are separate transactions.",
            source_path="qa/b013-assets/R011-B013_ASSET_CLOSURE.json", source_span=None,
    ...
    section_key = "r011/unit/source-label/contDist"
    unit_specs: list[dict[str, Any]] = [{
        "key": section_key, "type": "section", "title": "Continuous distributions",
        "parent": chapter_id, "order": 5,
        "source_meta": span_meta(context["main_authority"], 124925, 132799),
    ...
    for source_sub, target_sub in zip(source_subs, target_subs):
        key = f"r011/unit/b013/subsection-{source_sub['number']:02d}-{slug(source_sub['title'])}"
        subsection_keys.append(key)
        unit_specs.append({
            "key": key, "type": "subsection", "title": source_sub["title"],
            "parent": section_key, "order": source_sub["number"] * 100,
            "source_meta": rebase(source_sub["span"], context["main_authority"], 124925),
    ...
    for number in WORKED_EXAMPLES:
        ...
        unit_specs.append({
            "key": f"r011/unit/worked-example/ch03-sec3.5-{number:02d}", "type": "worked_example",
            "title": f"Section 3.5 worked example {number}", "parent": section_key, "order": 1000 + number * 10,
            "source_meta": rebase(source_row["span"], context["main_authority"], 124925),
    ...
    for number in GUIDED_EXERCISES:
        ...
        guide_key = f"r011/unit/guided-exercise/ch03-sec3.5-{number:02d}"
        unit_specs.append({
            "key": guide_key, "type": "guided_exercise", "title": f"Section 3.5 guided exercise {number}",
        ...
        unit_specs.append({
            "key": f"r011/unit/guided-solution/ch03-sec3.5-{number:02d}", "type": "solution",
            "title": f"Inline public answer to Section 3.5 guided exercise {number}",
            "parent": guide_key, "order": 1,
            "source_meta": rebase(source_answer["span"], context["main_authority"], 124925),
    ...
    for number in EOCE_EXERCISES:
        ...
        unit_specs.append({
            ...
            "source_path": "ch_probability/TeX/continuous_distributions.tex", "target_path": "repo/ch_probability/TeX/continuous_distributions.tex",
        ...
        if number in PUBLIC_ANSWERS:
            unit_specs.append({
                ...
                "source_meta": rebase(context["answer_source_spans"][number], context["answer_authority"], 24671),
    ...
    for index, (source_row, target_row) in enumerate(zip(context["main_source_spans"], context["main_target_spans"]), 1):
        ...
        if kind == "worked_example":
            unit_key = f"r011/unit/worked-example/ch03-sec3.5-{number:02d}"
        elif kind == "guided_exercise":
            unit_key = f"r011/unit/guided-exercise/ch03-sec3.5-{number:02d}"
        elif kind == "guided_inline_answer":
            unit_key = f"r011/unit/guided-solution/ch03-sec3.5-{number:02d}"
        else:
            unit_key = section_key
        segment_specs.append({
            "key": f"r011/segment/b013-main-{index:03d}", "kind": kind, "unit": unit_key, "order": index,
    ...
    for number in EOCE_EXERCISES:
        segment_specs.append({
            "key": f"r011/segment/b013-eoce-{number}", "kind": "end-of-section-exercise",
    ...
    for number in PUBLIC_ANSWERS:
        segment_specs.append({
            "key": f"r011/segment/b013-answer-{number}", "kind": "public_appendix_solution",
    ...
    for spec in segment_specs:
        ...
        localization_key = f"r011/localization/id-ID/b013-{spec['key'].rsplit('/', 1)[-1]}"
        ...
        records["localizations"].append(record(
            ...
            protected_tokens=g.protected_tokens(target_text), protected_token_delta={"authorized": True, "reason": "Terminal B013 translation QA proves structural, formula, numeric, label, reference, and authorized layout/localization deltas."},
    ...
    for order, (source_term, target_term, variants) in enumerate(TERM_SPECS, 1):
        key = f"r011/concept/b013/{slug(source_term)}"
        ...
        meta = rebase(span_meta(context["main_source"], occurrence, occurrence + len(source_term.encode("utf-8"))), context["main_authority"], 124925) if occurrence >= 0 else span_meta(context["main_authority"], 124925, 124926)
        records["concepts"].append(record(
            "concept", key, preferred_source_term=source_term,
            definition=f"Section 3.5 continuous-distribution concept indexed as {source_term}.",
        ...
        records["terms"].append(record(
            "term", f"r011/term/id-ID/b013/{slug(source_term)}", source_term=source_term,
            target_term=target_term, concept_id=concept_id, scope="probability / Chapter 3 Section 3.5",
            register="academic", variants=variants, rejected_forms=[],
            decision="Controlled B013 field-usage decision propagated to terminal candidate.",
            ...
            internal_witness_bytes_excluded=True, glossary_lock_status="propagated_to_terminal_b013_candidate",
            source_path="ch_probability/TeX/ch_probability.tex", source_span=schema_span(meta), source_sha256=meta["sha256"],
            **common_fields(resource_id, edition_id, concept_id, order, "id-ID", "language_reviewed", [upstream_rights, text_rights]),
        ))

    asset_ids: dict[str, str] = {}
    localized_asset_pairs: list[tuple[str, str]] = []
    asset_closure = context["asset_closure"]
    localization_producer = asset_closure["localization_producer"]
    producer_key = "r011/asset/b013/localized-figure-producer"
    producer_id = g.stable_id(producer_key)
    asset_ids[producer_key] = producer_id
    records["assets"].append(record(
        "asset", producer_key, asset_kind="localized_figure_producer",
        path=localization_producer["path"], bytes=localization_producer["bytes"],
        sha256=localization_producer["sha256"], media_type="text/x-python",
        target_locale="id-ID", deterministic_replay=True,
        figure_bindings=[figure["id"] for figure in asset_closure["assets"]],
        source_path=localization_producer["path"], source_span=None,
        source_sha256=localization_producer["sha256"],
        **common_fields(resource_id, edition_id, unit_ids[section_key], 1, "id-ID", "structurally_verified", [upstream_rights, asset_rights]),
    ))
    for order, figure in enumerate(asset_closure["assets"], 1):
        figure_id = figure["id"]
        source_pdf_key = f"r011/asset/b013/source-pdf/{figure_id}"
        source_r_key = f"r011/asset/b013/source-producer/{figure_id}"
        for key, kind, item, media, rights in (
            (source_pdf_key, "source_figure_pdf", figure["source_pdf"], "application/pdf", [upstream_rights]),
            (source_r_key, "source_r_producer", figure["source_generator"], "text/x-r-source", [upstream_rights]),
        ):
            ...
            records["assets"].append(record(
                ...
                media_type=media, dependencies=[], reader_visible_language="en" if kind == "source_figure_pdf" else "zxx",
                source_path=item["path"], source_span=None,
        ...
        item = figure["localized_pdf"]
        require(LANE / item["path"], {"bytes": int(item["bytes"]), "sha256": item["sha256"]})
        localized_key = f"r011/asset/b013/localized-pdf/{figure_id}"
        localized_id = g.stable_id(localized_key)
        asset_ids[localized_key] = localized_id
        localized_asset_pairs.append((producer_id, localized_id))
        records["assets"].append(record(
            "asset", localized_key, asset_kind="localized_figure_pdf", path=item["path"],
            bytes=item["bytes"], sha256=item["sha256"], media_type="application/pdf",
            target_locale="id-ID", reader_visible_language="id-ID", semantic_change=False,
            localization_producer_id=producer_id, source_asset_id=asset_ids[source_pdf_key],
            source_generator_asset_id=asset_ids[source_r_key],
            source_path=item["path"], source_span=None, source_sha256=item["sha256"],
            **common_fields(resource_id, edition_id, asset_ids[source_pdf_key], order, "id-ID", "visually_checked", [upstream_rights, asset_rights]),
        ))

    correction_ids: dict[str, str] = {}
    correction_rows = context["translation_qa"].get("source_corrections")
    if len(correction_rows) != 3:
        raise RuntimeError("B013 typed source-correction count changed")
    for order, item in enumerate(correction_rows, 1):
        key = f"r011/correction/b013-source-accessibility-{order:02d}"
        ...
        records["corrections"].append(record(
            ...
            correction_type="localized_accessibility_correction", summary=f"{item['id']}: {item['source_text']}",
            source_claim=item["source_text"], proposed_correction=item["correction"],
            rationale="High-confidence source accessibility/grammar defect corrected only in the Indonesian derivative.",
            disposition="applied_in_terminal_b013_candidate", confidence="high",
            evidence=f"{item['source_location']}; {context['inputs']['final_translation_qa']['path']}",
    ...
    for role, item in sorted(context["inputs"].items()):
        evidence_sources.append((role, f"evidence/b013/{role}-{Path(item['path']).name}", context["raw"][role], role, "visually_checked" if role in {"reader_pdf", "visual_qa"} else "built" if role in {"build_receipt", "source_manifest", "source_qa"} else "language_reviewed", [upstream_rights, text_rights] if role in {"main_fragment", "eoce_fragment", "public_answers_fragment", "assembled_main", "assembled_answers", "reader_pdf"} else []))
    evidence_sources.extend([
        ("terminal-contract", "evidence/b013/R011-B013_TERMINAL_INPUTS.json", require(TERMINAL_CONTRACT, TERMINAL_CONTRACT_IDENTITY), "terminal_input_contract", "structurally_verified", []),
        ("requirements", "evidence/b013/R011-B013_BACKEND_INPUT_REQUIREMENTS.json", require(REQUIREMENTS, REQUIREMENTS_IDENTITY), "backend_requirements", "structurally_verified", []),
        ("backend-generator", "evidence/tools/generate_backend_b013.py", require(SCRIPT_PATH), "backend_generator", "structurally_verified", []),
    ])
    validator_path = SCRIPTS / "validate_backend_b013.py"
    if validator_path.is_file():
        evidence_sources.append(("backend-validator", "evidence/tools/validate_backend_b013.py", require(validator_path), "backend_validator", "structurally_verified", []))
    artifact_ids: dict[str, str] = {}
    for order, (role, relative, raw, kind, state, rights) in enumerate(evidence_sources, 1):
        auxiliary[relative] = raw
        key = f"r011/artifact/b013-{slug(role)}"
        artifact_ids[role] = g.stable_id(key)
        records["artifacts"].append(record(
            "artifact", key, artifact_kind=kind, path=f"qa/b013-backend-final/exports/{relative}",
            bytes=len(raw), sha256=sha256_bytes(raw), result="exact terminal B013 input or isolated backend evidence",
            toolchain=context["build_receipt"].get("toolchain") if role in {"build_receipt", "reader_pdf"} else None,
            build_receipt="qa/b013-backend-final/exports/evidence/b013/build_receipt-CANDIDATE_BUILD_QA_B013.json" if role == "reader_pdf" else None,
            source_path=None, source_span=None, source_sha256=None, provenance=PROVENANCE,
            **common_fields(resource_id, edition_id, edition_id, order, "id-ID" if role in {"main_fragment", "eoce_fragment", "public_answers_fragment", "assembled_main", "assembled_answers", "preface", "reader_pdf"} else "zxx", state, rights),
        ))

    qa_specs = [
        ("base-preservation", "topology", "terminal-contract", "All 4,276 admitted B012 records preserve exact canonical bytes and stable identities."),
        ("translation", "language", "final_translation_qa", "Complete Section 3.5, three worked examples, two guided exercises and inline answers, EoCE 37--38, public answer 37, and O001 gap 38 are terminally bound."),
        ("terminology", "language", "terminology_qa", "Six controlled B013 terms are bound to the reused field-source decision evidence and edition glossary."),
        ("asset-closure", "asset", "asset_closure_qa", "Five source PDF/R pairs, five localized PDF/producer bindings, component rights, and label-only semantics are explicit."),
        ("source-overlay", "source", "source_qa", "The isolated B013 source overlay and manifest are exact."),
        ("deterministic-build", "build", "build_receipt", "Two independent B013 PDF replays are byte-identical and bind the 427-page reader."),
    ...
    for order, (suffix, qa_type, witness, detail) in enumerate(qa_specs, 1):
        ...
        records["qa_events"].append(record(
            ...
            witness_path=f"qa/b013-backend-final/exports/" + next(relative for role, relative, *_rest in evidence_sources if role == witness),
    ...
    for spec in unit_specs:
        ...
        add_relation(records, f"r011/relation/b013-contains-{slug(spec['key'])}", "contains", parent, unit_ids[spec["key"]], relation_order, "B013 hierarchy", resource_id, edition_id); relation_order += 1
    for spec in segment_specs:
        add_relation(records, f"r011/relation/b013-unit-segment-{slug(spec['key'])}", "contains", unit_ids[spec["unit"]], segment_ids[spec["key"]], relation_order, "unit contains exact source segment", resource_id, edition_id); relation_order += 1
        add_relation(records, f"r011/relation/b013-localizes-{slug(spec['key'])}", "localizes", localization_ids[spec["key"]], segment_ids[spec["key"]], relation_order, "terminal id-ID localization", resource_id, edition_id); relation_order += 1
    for number in GUIDED_EXERCISES:
        add_relation(records, f"r011/relation/b013-guided-answer-{number}", "answers", unit_ids[f"r011/unit/guided-solution/ch03-sec3.5-{number:02d}"], unit_ids[f"r011/unit/guided-exercise/ch03-sec3.5-{number:02d}"], relation_order, "public inline answer", resource_id, edition_id); relation_order += 1
    for number in EOCE_EXERCISES:
        ...
        add_relation(records, f"r011/relation/b013-exercise-answer-{number}", "answers" if number in PUBLIC_ANSWERS else "requires_companion_answer", linked if number in PUBLIC_ANSWERS else exercise, exercise if number in PUBLIC_ANSWERS else linked, relation_order, "public upstream answer" if number in PUBLIC_ANSWERS else "explicit O001 gap; restricted solution not accessed", resource_id, edition_id); relation_order += 1
    add_relation(records, "r011/relation/b013-section-follows-b012", "precedes", previous_section, unit_ids[section_key], relation_order, "source order Section 3.4 to 3.5", resource_id, edition_id); relation_order += 1
    for source_term, _target, _variants in TERM_SPECS:
        term_id = g.stable_id(f"r011/term/id-ID/b013/{slug(source_term)}")
        add_relation(records, f"r011/relation/b013-lexicalizes-{slug(source_term)}", "lexicalizes", term_id, concept_ids[source_term], relation_order, "terminal B013 terminology", resource_id, edition_id); relation_order += 1
        add_relation(records, f"r011/relation/b013-covers-{slug(source_term)}", "covers", unit_ids[section_key], concept_ids[source_term], relation_order, "Section 3.5 concept index", resource_id, edition_id); relation_order += 1
    for key, asset_id in asset_ids.items():
        add_relation(records, f"r011/relation/b013-asset-{slug(key)}", "uses_asset", unit_ids[section_key], asset_id, relation_order, "B013 figure/code/data closure", resource_id, edition_id); relation_order += 1
    for producer_id, localized_id in localized_asset_pairs:
        figure_key = next(key for key, value in asset_ids.items() if value == localized_id)
        add_relation(records, f"r011/relation/b013-produces-{slug(figure_key)}", "produces", producer_id, localized_id, relation_order, "deterministic localized figure/producer pair", resource_id, edition_id); relation_order += 1
    for role, artifact_id in artifact_ids.items():
        add_relation(records, f"r011/relation/b013-artifact-{slug(role)}", "documents", artifact_id, edition_id, relation_order, "exact B013 evidence", resource_id, edition_id); relation_order += 1
    for suffix, qa_id in qa_ids.items():
        witness = next(item[2] for item in qa_specs if item[0] == suffix)
        add_relation(records, f"r011/relation/b013-qa-{slug(suffix)}", "validates", qa_id, artifact_ids[witness], relation_order, "typed B013 QA event", resource_id, edition_id); relation_order += 1
    for key, correction_id in correction_ids.items():
        add_relation(records, f"r011/relation/b013-correction-{slug(key)}", "corrects", correction_id, unit_ids[section_key], relation_order, "typed source accessibility correction", resource_id, edition_id); relation_order += 1
    add_relation(records, "r011/relation/b013-rights-text", "governs", text_rights, unit_ids[section_key], relation_order, "localized text rights", resource_id, edition_id); relation_order += 1
    add_relation(records, "r011/relation/b013-rights-assets", "governs", asset_rights, unit_ids[section_key], relation_order, "localized asset rights", resource_id, edition_id); relation_order += 1
...
def build_payloads() -> tuple[dict[str, bytes], dict[str, Any]]:
    ...
    manifest.update({
        "backend_id": g.stable_id("r011/backend/b013-final-isolated"),
        "backend_name": "r011-openintro-statistics-id-b013-final-isolated",
        "boundary_id": BOUNDARY_ID,
        "workflow_id": WORKFLOW_ID, "recorded_at": RECORDED_AT,
        "scope": "Complete Section 3.5 Continuous distributions / Distribusi kontinu through EoCE 37--38 and public answer 37, ending at Chapter 3 EOF before normalDist.",
        "base_preservation": {
            ...
            "policy": "Every admitted B012 typed record retains exact canonical record bytes and stable identity; live backend is read-only.",
        },
        "source_application": {
        ...
        "new_b013_record_counts": new_counts, "new_b013_record_count": sum(new_counts.values()),
        "topology": {
            ...
            "assets": new_counts["assets"], "relations_emitted": context["relation_count"], "next_source_anchor": "normalDist",
        },
        "asset_closure": {
            "source_figure_producer_pairs": 5, "localized_pdf_producer_pairs": 5,
            "shared_localization_producer": context["context"]["inputs"]["localization_producer"],
        ...
        "stage_state": {
            "status": "isolated_b013_final_generated", "boundary_admitted": False,
...
def validate_payloads(payloads: dict[str, bytes], context: dict[str, Any]) -> dict[str, Any]:
    ...
    for name in RECORD_PATHS:
        ...
        if any(after.get(record_id) != raw for record_id, raw in before.items()):
            raise RuntimeError(f"B012 base record bytes changed in {name}")
    ...
    id_set = set(ids)
    for row in all_rows:
        for field in ("resource_id", "edition_id", "parent_id", "concept_id", "source_segment_id", "subject_id", "affected_id", "from_id", "to_id", "unit_id", "witness_artifact_id", "source_asset_id", "source_generator_asset_id", "localization_producer_id"):
            value = row.get(field)
    ...
    for row in new_rows:
        ...
        if errors:
            raise RuntimeError(f"B013 record schema failure {row['stable_key']}: {errors[0].message}")
    ...
    required_deltas = {
        "units": 14, "segments": 14, "localizations": 14,
        "concepts": 6, "terms": 6, "assets": 16, "rights": 2,
        "corrections": 3, "qa_events": 10, "artifacts": 30,
        "relations": 125,
    }
    if any(deltas[name] != expected for name, expected in required_deltas.items()):
        raise RuntimeError(f"B013 required class delta mismatch: {deltas}")
    gaps = [row for row in records["units"] if row.get("boundary_id") == BOUNDARY_ID and row.get("unit_type") == "companion_gap"]
    if len(gaps) != 1 or any(row.get("source_solution_used") is not False or row.get("source_path") is not None for row in gaps):
        raise RuntimeError("B013 explicit O001 gap closure failed")
...
def write_output(output: Path, payloads: dict[str, bytes]) -> dict[str, Any]:
    ...
    if resolved.parent != STAGE_ROOT.resolve() or resolved.name not in {"run-a", "run-b", "exports"}:
        raise RuntimeError(f"refusing output outside exact B013 final runs: {resolved}")
    if resolved.exists():
        existing = inventory(resolved)
        expected_paths = set(payloads)
        observed_paths = {item["path"] for item in existing["inventory"]}
        if observed_paths - expected_paths:
            raise RuntimeError(f"refusing to replace B013 final run with unexpected files: {sorted(observed_paths - expected_paths)[:3]}")
...
def compare_runs(left: Path, right: Path) -> dict[str, Any]:
    ...
    if left_inventory != right_inventory:
        raise RuntimeError("independent B013 final run inventories differ")
...
def run(output: Path | None, validate_only: Path | None) -> dict[str, Any]:
    ...
    return {
        "boundary_id": BOUNDARY_ID, "status": "PASS_B013_FINAL_ISOLATED_BACKEND",

# R011-B010 publication checklist
...
1. Admission: run self-test and readiness, execute the guarded B010 transaction once authorized, verify exact admitted state, then emit the post-admission receipt. Admission updates source/assets/PDF/backend and root README/CITATION/license/checkpoint/cursor in one rollback-protected transaction.
...
```powershell
python -m py_compile scripts\admit_b010.py scripts\write_post_admission_verification_b010.py scripts\bind_release_inputs_b010.py scripts\release_b010_common.py scripts\package_release_b010.py scripts\publish_b010.py scripts\publish_zenodo_b010.py scripts\publish_figshare_b010.py scripts\publish_github_b010.py
python scripts\admit_b010.py --self-test
python scripts\admit_b010.py --check-readiness
```
...
```powershell
python scripts\admit_b010.py --promote
python scripts\admit_b010.py --verify-admitted
python scripts\write_post_admission_verification_b010.py
python scripts\bind_release_inputs_b010.py --check
python scripts\bind_release_inputs_b010.py --bind
python scripts\package_release_b010.py --self-check
python scripts\package_release_b010.py --package
```

Required gates are `verified_admitted_exact`, `PASS_VERIFIED_ADMITTED_EXACT`, `READY_FOR_PACKAGING`, `PASS_INERT_FAIL_CLOSED`, and `PASS_PACKAGED_VERIFIED`. Use `--replace` with the packager only after inspecting the exact nine B010 target names if a retry encounters pre-existing package outputs.

Only after the package passes, execute the already authorized public transaction in this order:

```powershell
python scripts\publish_zenodo_b010.py --self-check
python scripts\publish_zenodo_b010.py --publish
python scripts\publish_figshare_b010.py --self-check
python scripts\publish_figshare_b010.py --publish
python scripts\publish_github_b010.py --self-check
python scripts\publish_github_b010.py --publish
```

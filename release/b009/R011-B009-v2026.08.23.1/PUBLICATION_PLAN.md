# R011-B009 publication plan (inert until terminal admission)
...
1. Bind exact B009 boundary, isolated/admitted backend, admission, post-admission readback, and promoted-PDF identities in `RELEASE_INPUTS.json`; change status to `READY_FOR_PACKAGING` only when every assertion is true.
2. Run the B009 packager. It reconstructs and verifies the B009 source closure, excludes only the recorded nonpublishable component, builds deterministic source/backend ZIPs, places the PDF first, emits exact hashes, and verifies the package.
...
Self-check mode reads no credentials, performs no network request, invokes no Git command, creates no package, and publishes nothing. Publisher `--publish` remains fail-closed until every terminal identity is filled and the package is verified.
